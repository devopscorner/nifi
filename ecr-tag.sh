...
main() {
  # set_tag [AWS_ACCOUNT] devopscorner/nifi [alpine|ubuntu] [version|latest|tags] [custom-tags]
  # set_tag [AWS_ACCOUNT] devopscorner/nifi-registry [alpine|ubuntu] [version|latest|tags] [custom-tags]
  set_tag $1 $2 $3 $4 $5
  docker_tag
  echo ''
  echo '-- ALL DONE --'
